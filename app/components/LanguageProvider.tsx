...
type ContentValues = Partial<Record<SiteLanguage, string>>;
type ContentOverrides = Record<string, ContentValues>;

const LanguageContext = createContext<LanguageContextValue | null>(null);
const originalText = new WeakMap<Text, string>();
const originalAttributes = new WeakMap<Element, Record<string, string>>();
const supported: SiteLanguage[] = ["tr", "ru", "en", "ro"];
const sourceByDisplayedText = new Map<string, string>();

for (const language of ["en", "ru", "ro"] as const) {
  for (const [source, translated] of Object.entries(translations[language])) {
    sourceByDisplayedText.set(source, source);
    if (!sourceByDisplayedText.has(translated)) sourceByDisplayedText.set(translated, source);
  }
}
...
function remoteValue(value: string, language: SiteLanguage, overrides: ContentOverrides) {
  const source = sourceByDisplayedText.get(value) ?? value;
  const saved = overrides[source]?.[language];
  return typeof saved === "string" && saved.trim() ? saved : null;
}

function translatedValue(value: string, language: SiteLanguage, overrides: ContentOverrides) {
  const saved = remoteValue(value, language, overrides);
...
function translateText(node: Text, language: SiteLanguage, overrides: ContentOverrides) {
  ...
  if (!isProtected && !originalText.has(node)) originalText.set(node, initial);
  const value = initial.trim();
  if (!value) return;
  const translated = isProtected ? remoteValue(value, language, overrides) : translatedValue(value, language, overrides);
...
function translateAttributes(element: Element, language: SiteLanguage, overrides: ContentOverrides) {
  ...
  for (const name of attributes) {
    ...
    if (!source) continue;
    const next = translatedValue(source, language, overrides);
    if (current !== next) element.setAttribute(name, next);
  }
  if (Object.keys(saved).length) originalAttributes.set(element, saved);
}

function translateTree(root: Node, language: SiteLanguage, overrides: ContentOverrides) {
  const rootElement = root.nodeType === Node.TEXT_NODE ? root.parentElement : root as Element;
  if (rootElement?.closest("script, style, textarea")) return;
  const walker = document.createTreeWalker(root, NodeFilter.SHOW_ELEMENT | NodeFilter.SHOW_TEXT, {
    acceptNode(node) {
      const parent = node.nodeType === Node.TEXT_NODE ? node.parentElement : node as Element;
      return parent?.closest("script, style, textarea") ? NodeFilter.FILTER_REJECT : NodeFilter.FILTER_ACCEPT;
    },
  });
  let node: Node | null = root;
  while (node) {
    if (node.nodeType === Node.TEXT_NODE) translateText(node as Text, language, overrides);
    else if (node.nodeType === Node.ELEMENT_NODE) translateAttributes(node as Element, language, overrides);
...
export function LanguageProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    fetch("/api/site-content", { cache: "no-store" })
      .then(async (response) => response.ok ? response.json() as Promise<{ overrides?: ContentOverrides }> : { overrides: {} })
      .then((result) => { if (active) setContentOverrides(result.overrides ?? {}); })
      .catch(() => undefined);
    return () => { active = false; };
  }, []);
  ...
  useEffect(() => {
    ...
    translateTree(document.body, language, contentOverrides);
    const observer = new MutationObserver((mutations) => {
      for (const mutation of mutations) {
        if (mutation.type === "characterData") translateText(mutation.target as Text, language, contentOverrides);
        mutation.addedNodes.forEach((node) => translateTree(node, language, contentOverrides));
      }
    });
